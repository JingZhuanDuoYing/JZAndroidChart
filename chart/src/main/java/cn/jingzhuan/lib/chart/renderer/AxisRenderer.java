...
public class AxisRenderer implements Renderer {
  ...
  private static void computeAxisStopsX(float start, float stop, AxisX axis,
      AxisAutoValues autoValues) {
  ...
  public void drawGridLines(Canvas canvas) {
    ...
    if (mAxis.isEnable() && mAxis.isGridLineEnable() && mAxis.getGridCount() > 0) {
      ...
      if (mAxis.getDashedGridIntervals() != null && mAxis.getDashedGridPhase() > 0) {
        mGridPaint.setPathEffect(
            new DashPathEffect(mAxis.getDashedGridIntervals(), mAxis.getDashedGridPhase()));
      }
      ...
      if (mAxis instanceof AxisX) {
        ...
        for (int i = 1; i < count; i++) {
          if (mAxis.getGirdLineColorSetter() != null) {
            mGridPaint.setColor(
                mAxis.getGirdLineColorSetter().getColorByIndex(mAxis.getGridColor(), i));
          }
          canvas.drawLine(
