...
public abstract class Chart extends BitmapCachedChart {
  ...
  private void init(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    mTouchPointChangeListeners =
        Collections.synchronizedList(new ArrayList<OnTouchPointChangeListener>());
    mOnViewportChangeListeners =
        Collections.synchronizedList(new ArrayList<OnViewportChangeListener>());
  ...
  private void fling(int velocityX) {
    ...
    mScrollerStartViewport.set(mCurrentViewport);
    int startX =
        (int) (mSurfaceSizeBuffer.x * (mScrollerStartViewport.left - Viewport.AXIS_X_MIN) / (
            Viewport.AXIS_X_MAX - Viewport.AXIS_X_MIN));
    mScroller.forceFinished(true);
    mScroller.fling(
        startX,
  ...
  public void removeOnTouchPointChangeListener(
  ...
  /**
   * 设置是否开启触摸的时候，可见区域是否跟随手指移动
   *
   * @param draggingToMoveEnable 默认为true
   */
