 ...
 * @since 2023-08-04
 */
public class JZLineScatterDraw extends BaseDraw<ScatterValue, ScatterDataSet> {
    ...
    public JZLineScatterDraw(Chart chart) {
        super(chart);

    }

    @Override
    public void drawDataSet(Canvas canvas, ChartData<ScatterDataSet> chartData, ScatterDataSet dataSet) {
        clearTemporaryData();
        for (ScatterDataSet scatterDataSet : chartData.getDataSets()) {
            drawScatterDataSet(canvas, scatterDataSet, chartData.getLeftMax(), chartData.getLeftMin(), chartData.getRightMax(), chartData.getRightMin());
        }
    }

    private void drawScatterDataSet(Canvas canvas, final ScatterDataSet dataSet, float leftMax, float leftMin, float rightMax, float rightMin) {
