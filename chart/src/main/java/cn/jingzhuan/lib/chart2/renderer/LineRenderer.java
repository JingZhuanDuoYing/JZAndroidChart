...
public class LineRenderer extends AbstractDataRenderer<LineDataSet> {
  ...
  /**
   * 当显示图表view为{@linkplain LineChart }时，true
   */
  ...
  private void drawDataSet(Canvas canvas, final LineDataSet lineDataSet,
    ...
    final float step = (valueCount > 1 && onlyLines) ?
        (mContentRect.width() * scale / (valueCount - 1))
        : (mContentRect.width() * scale / valueCount);


    /**
     * 得到一个绘制坐标当x数值。
     * mViewport.left * mContentRect.width() * scale 计算可视区域当变化
     */
    final float startX = mContentRect.left + (onlyLines ? 0f : step * 0.5f)
        - mViewport.left * mContentRect.width() * scale;
    ...
    if (mChartAnimator.getPhaseX() > 0) {
      lastIndex =
          (int) (Math.floor(lineDataSet.getValues().size() * mChartAnimator.getPhaseX()) - 1);
    }
